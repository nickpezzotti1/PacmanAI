...
class Maze:
    ...
    def __init__(self, state, delta, gamma, min_distance_from_ghosts, food, ghost, caps, empty):
        ...
        self._fill(state)
        self.MIN_DISTANCE_FROM_GHOST = int(math.log(3 * max(len(self.map), len(self.map[0])))/math.log(3)) + 1
    ...
    def _flood_fill(self, distances, row, col, val):
        if row < 0 or row >= len(self.map) or col < 0 or col >= len(self.map[row]) or (distances[row][col] != -1 and val >= distances[row][col]) or self.map[row][col] == MazeEntity.WALL or val > self.MIN_DISTANCE_FROM_GHOST:
            return
